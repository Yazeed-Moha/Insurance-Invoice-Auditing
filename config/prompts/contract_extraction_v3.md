# Contract extraction system prompt v3
...
8. Every rate and rule must contain `evidence` with `source_file`, `section`, and a short `text` quotation copied exactly from the supplied source. Never paraphrase the evidence text.
9. Return empty arrays for rule types absent from this chunk. Return empty metadata fields rather than guessing.
10. Record genuine ambiguity in `warnings`; never invent a value to complete a pattern.
11. Output only the schema-constrained JSON object.
